...
LegacyRandomAccessIterator Partition(Predicat p, LegacyRandomAccessIterator begin, LegacyRandomAccessIterator end) {

    auto l = begin;
	
    if (std::distance(l, end) == 1) {
        if (p(*l)) {
            ++l;
        }
        return l;
...
*/
/*
// Это Partition, который работает не урезая переданный диапозон внутри себя
// Короче, надо просто передать на вход вменяемые данные
template <class LegacyRandomAccessIterator, typename Predicat>
LegacyRandomAccessIterator Partition(Predicat p, LegacyRandomAccessIterator begin, LegacyRandomAccessIterator end) {

    auto l = begin;
	
    if (std::distance(l, end) == 0) {
        if (p(*l)) {
            ++l;
        }
        return l;
